	...
	public static void run() throws IOException, InterruptedException {
		...
		while (InterfxOverviewController.running) {
			if (!fifoQueue.isEmpty()) {
				...
				if (instr == 0) {
					MotorControl_v2.steeringMotor.rotateTo(0, true);
				}
				if (instr == 9) {
					MotorControl_v2.autoRun();
				}
				if (instr == 98)
					getParams();
				if (instr == 99)
					getBatteryLevel();
				if (instr >= -160 && instr < -100) {
					MotorControl_v2.steeringMotor.rotateTo(Math.round(-((instr + 100) * ratio)), true);
					startedSteering = true;
				}
				if (instr >= 100 && instr <= 160) {
					MotorControl_v2.steeringMotor.rotateTo(Math.round(-((instr - 100) * ratio)), true);
					startedSteering = true;
				}
					...
					try {
						...
						if (localAngle >= 4)
							...
							if (localAngle <= -4)
								...
								MotorControl_v2.steeringStop();
						}
					} catch (IndexOutOfBoundsException e) {
						InterfxOverviewController.message.set("Index out of bounds\n");
					}
				}
			}
			Thread.sleep(10);
